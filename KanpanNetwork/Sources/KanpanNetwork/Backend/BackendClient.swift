...
///
/// 后端只有网关那两台（主在前、备在后），跟行情线路选直连还是网关无关——直连线路下
/// 板块历史、供应量这些也只能问后端。主机表从 `RouteResolver.backend` 拿，不在各页面
/// 自己拼名单、自己建 `URLSession`。
///
/// 规则：
/// - 主、备按顺序试；连不上 / 超时 / 5xx / 429 换下一台，4xx（除 429）是这一笔本身的问题，换主机也一样，直接报。
