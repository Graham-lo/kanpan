...
///   HTTP-date，两种都解（`BinanceError.retryAfterSeconds`）。
/// - `code`：币安错误体里的 `code`（`-1003` 是超频）。
...
public struct BinanceError: Error, Sendable, Equatable, CustomStringConvertible {
  ...
  public static func blocked(seconds: TimeInterval, upstreamStatus: Int = 429,
                             url: String? = nil) -> BinanceError {
    BinanceError(status: 429, code: nil,
  ...
  /// 必须**结束本轮**：`BinanceREST.fetch` 那一层已经按上游给的截止时间处理过了，
