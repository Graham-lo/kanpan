...
struct DefaultFavoritesTests {
  ...
  private static let mixedCatalog: [SymbolInfo] = SymbolFixtures.catalog + [
    SymbolInfo(symbol: "TSLAUSDT", base: "TSLA", pricePrecision: 2, tickSize: 0.01,
               underlyingType: "STOCK"),
    SymbolInfo(symbol: "XAUUSDT", base: "XAU", pricePrecision: 2, tickSize: 0.01,
               underlyingType: "COMMODITY"),
    // 字段缺了的：分类那一层不猜，所以它也不算币。
    SymbolInfo(symbol: "MYSTERYUSDT", base: "MYSTERY", pricePrecision: 2, tickSize: 0.01),
  ]

  private static let mixedTickers: [Ticker] = SymbolFixtures.tickers + [
    Ticker(symbol: "TSLAUSDT", last: 410, changePercent: 2, high: 420, low: 400, quoteVolume: 9.9e10),
    Ticker(symbol: "XAUUSDT", last: 3300, changePercent: 0.2, high: 3310, low: 3290, quoteVolume: 8.8e10),
    Ticker(symbol: "MYSTERYUSDT", last: 1, changePercent: 0, high: 1, low: 1, quoteVolume: 7.7e10),
  ]

  @Test("三个锚在前，后面跟成交额前五")
  func anchorsThenHot() {
    let picked = DefaultFavorites.pick(catalog: SymbolFixtures.catalog, tickers: SymbolFixtures.tickers)
    #expect(picked.prefix(3) == ["BTCUSDT", "ETHUSDT", "SOLUSDT"])
    #expect(picked.count == 8)
    // 锚之外按成交额降序：BNB 6.4e8 > XRP 5.92e8 > DOGE 4.03e8 > 1000PEPE 2.88e8 > AVAX 1.02e8
    #expect(Array(picked.dropFirst(3)) == ["BNBUSDT", "XRPUSDT", "DOGEUSDT", "1000PEPEUSDT", "AVAXUSDT"])
  }
  ...
  func noneCoinContractsAreExcluded() {
    ...
    #expect(!picked.contains("TSLAUSDT"))
    #expect(!picked.contains("XAUUSDT"))
    #expect(!picked.contains("MYSTERYUSDT"))
    #expect(picked.count == 8)
  }
  ...
  func dedupesByCoin() {
    let catalog = SymbolFixtures.catalog + [
      SymbolInfo(symbol: "BTCUSDC", base: "BTC", quote: "USDC", pricePrecision: 2, tickSize: 0.1,
                 underlyingType: "COIN"),
      SymbolInfo(symbol: "PEPEUSDT", base: "PEPE", pricePrecision: 7, tickSize: 0.0000001,
                 underlyingType: "COIN"),
    ]
    let tickers = SymbolFixtures.tickers + [
      Ticker(symbol: "BTCUSDC", last: 76_800, changePercent: 1, high: 1, low: 1, quoteVolume: 1.0e8),
      Ticker(symbol: "PEPEUSDT", last: 0.0000074, changePercent: -1, high: 1, low: 1, quoteVolume: 3.0e8),
    ]
    let picked = DefaultFavorites.pick(catalog: catalog, tickers: tickers)
    #expect(picked.filter { $0.hasPrefix("BTC") } == ["BTCUSDT"])   // 成交额大的那条留下
    // PEPE 与 1000PEPE 是同一个币：只留成交额大的 PEPEUSDT（3.0e8 > 2.88e8）
    #expect(picked.filter { $0.contains("PEPE") } == ["PEPEUSDT"])
  ...
  func skipsDelisted() {
    ...
    #expect(!picked.contains("BNBUSDT"))
