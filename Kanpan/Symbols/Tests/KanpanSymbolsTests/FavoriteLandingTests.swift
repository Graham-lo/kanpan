...
struct FavoriteLandingTests {
  ...
  func landsInCurrentGroup() throws {
    ...
    m.addFavorite("BTCUSDT")
    #expect(m.prefs.groupForSymbol["BTCUSDT"] == short)
    #expect(m.prefs.groups.map(\.name) == ["短线", "长线"])   // 没有多出一个「加密」

    m.selectedGroupSource = { long }
    m.addFavorite("ETHUSDT")
    #expect(m.prefs.groupForSymbol["ETHUSDT"] == long)
    #expect(m.prefs.favorites(in: short) == ["BTCUSDT"])
  }
  ...
  func landsInDefaultGroup() throws {
    ...
    m.addFavorite("SOLUSDT")
    #expect(m.prefs.groupForSymbol["SOLUSDT"] == first)
  }
  ...
  func firstEverFavorite() {
    ...
    #expect(m.prefs.groupForSymbol["BTCUSDT"] == m.prefs.groups.first?.id)
  }
  ...
  func starDoesNotDisturbTheList() throws {
    ...
    #expect(m.toggleFavorite("ETHUSDT"))
