...
struct ChartHandoffTests {
  ...
  func symbolAndInterval() {
    let info = ChartHandoff.userInfo(symbol: "BTCUSDT", interval: "4h")
    #expect(ChartHandoff.url(from: info)?.absoluteString == "hkline://symbol/BTCUSDT?interval=4h")
    #expect(ChartHandoff.link(from: info) == .symbol("BTCUSDT", interval: "4h"))
    #expect(ChartHandoff.link(from: ChartHandoff.userInfo(symbol: "ETHUSDT", interval: "1M")) == .symbol("ETHUSDT", interval: "1M"))
    #expect(ChartHandoff.link(from: ChartHandoff.userInfo(symbol: "1000PEPEUSDT", interval: "1m")) == .symbol("1000PEPEUSDT", interval: "1m"))
  }
  ...
  func survivesUserActivityRoundTrip() throws {
    ...
    sent.addUserInfoEntries(from: ChartHandoff.userInfo(symbol: "SOLUSDT", interval: "15m"))
    ...
    #expect(ChartHandoff.link(from: received) == .symbol("SOLUSDT", interval: "15m"))
  }
  ...
  func rejects() {
    ...
    // 没有周期也能接：只开品种。
    #expect(ChartHandoff.link(from: ["symbol": "btcusdt"]) == .symbol("BTCUSDT", interval: nil))
  }
}
