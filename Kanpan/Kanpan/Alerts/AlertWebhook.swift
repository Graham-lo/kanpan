...
///
/// 谁发（2026-09-26 起）：**登录着就只由服务端发。** 本机 `AlertEngine` 判响之后照常把
/// 「已触发」同步上去，服务端 `alerts.rs` 在物化这条提醒、看到它从 `active` 变成 `fired`
/// 的那一刻替它发；服务端自己判响的那一次也是它发。两条路在服务端同一道闸
/// （同步咨询锁 + `status='active'`）里去重，一件事只发一封。以前本机判响、本机也发，
/// 而服务端评估器常常同时判响，同一次到价对面收到两封。
///
/// 这里的 `fire` 只剩一种情形：**没登录**。那时没有账号同步、服务端根本不知道这条提醒，
/// 只能本机发，而且只发本机判响的那一次（`AlertStore.firedLocally`）。「发一条测试」
/// （`test`）不属于触发，照旧在本机直接发。
enum AlertWebhook {
