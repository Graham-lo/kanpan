...
struct ChartPanel: View {
  ...
  var body: some View {
    PanelSheet(title: "图表", subtitle: "画法与显示 · 配色仍归风格") {

      PanelGroupTitle(text: "布局与读数")
      PanelRow(name: "竖屏高度", meta: "主副图一起调整，下方指标可滚动查看") {
      ...
      switchRow("主轴允许翻转", "点击主价格轴改变方向", prefs.allowMainInversion) { $0.allowMainInversion = $1 }
      switchRow("副轴允许翻转", "各副图独立改变方向", prefs.allowSubInversion) { $0.allowSubInversion = $1 }
      switchRow("指标区域自适应", "为主图图例预留空间", prefs.adaptiveIndicators) { $0.adaptiveIndicators = $1 }
      switchRow("简化指标数值", "使用万、亿等单位", prefs.compactValues) { $0.compactValues = $1 }

      PanelGroupTitle(text: "类型")
      PanelRow(name: "画法", meta: "平均K线把毛刺抹平，看的是趋势——代价是实体不再是真实开收价",
               divider: false) {
      ...
      PanelRow(name: "网格", meta: "所有风格共用这项设置") {
      ...
      PanelRow(name: "阳线", meta: "空心只描边，密集时更透气") {
      ...
      PanelRow(name: "拖动位置", meta: "调整后续右侧留白，切换时保留当前历史位置") {
      ...
      PanelRow(name: "蜡烛位置", meta: "主图区里蜡烛整体往上或往下挪，大小不变", divider: false) {
      ...
      switchRow("实时价格线", "最新价横线和右轴上的价格胶囊", prefs.lastLine) { $0.lastLine = $1 }
      ...
      switchRow("至今涨幅", "十字线打开时，多报选中那根到最新价的涨跌幅", prefs.sinceChange) {
        $0.sinceChange = $1
      }
      .accessibilityIdentifier("chart.sinceChange")
      switchRow("画线显示", "只是不画，画好的线不会删", prefs.showDrawings, divider: false) {
        $0.showDrawings = $1
      }
      .accessibilityIdentifier("chart.showDrawings")

      PanelNote(markdown: "所有风格共用尺寸、坐标和手势。副图高度也可在指标面板单独调整。")
    }
