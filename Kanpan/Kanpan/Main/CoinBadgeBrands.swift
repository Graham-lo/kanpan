...
extension CoinSpec {
  ...
  private static let software: [String: CoinSpec] = [
    ...
    // GitLab 那只狐狸本来就是几块成角的碎片拼的，18px 上拼不出脸，就只留碎片本身：
    // 三块一样大的直角碎片，卡在左上、左下、右下三个角上，右上角空着——空出来的那一角
    // 就是它的记号，数得清、不对称。原来那只狐狸在 18px 上塌成「两条斜臂在底下收拢成
    // 一个 V」，和 Visa 那枚翼形 V 同一个骨架。
    "GTLB": CoinSpec(from: "#F0A47A", to: "#C25A18", mark: fill([
      "M2.8 2.8h8L2.8 10.8z",
      "M2.8 13.2v8h8z",
      "M13.2 21.2h8v-8z"])),
  ...
  private static let asia: [String: CoinSpec] = [
    ...
    "CSOPSAMSUNG2L": CoinSpec(from: "#9FC0E8", to: "#2A44B8", mark: fill([
      ...
      "M7 20h10v2.1H7z"])),
    // SK 海力士：一块带键的拼片——顶上啃掉一个方口，右边伸出一个方榫，它是把系统拼完整的
    // 那一块。原来那片「方环套芯」在 18px 上就是一个套着芯的环，和以太经典那颗描边钻石撞；
    // 存储介质这一格已经被美光的条、闪迪的卡、长鑫的颗粒、兆易的裸片占满，不能再画一块，
    // 所以这里换成一个抽象但没人占过的轮廓——方口和方榫都是直角，18px 下不会糊。
    "SKHYNIX": CoinSpec(from: "#EF8080", to: "#C0302A", mark: fill([
      "M3.4 4.2H6.6V8.2H11.4V4.2H16.2V9.2H20.2V14.8H16.2V19.8H3.4Z"])),
    // SKHY 是 SK 海力士的 ADR，和 SKHYNIX 是同一家公司在币安上的两个合约，**故意共用同一个
    // 记号**——和 HK0700/TENCENT、MATIC/POL 一样。下一轮清点撞脸时别把这一对当撞车拆掉。
    "SKHY": CoinSpec(from: "#EF8080", to: "#C0302A", mark: fill([
      "M3.4 4.2H6.6V8.2H11.4V4.2H16.2V9.2H20.2V14.8H16.2V19.8H3.4Z"])),

    "CSOPSKHYNIX2L"
: CoinSpec(from: "#F09090", to: "#C84038", mark: .parts([
...
extension CoinSpec {
  ...
  private static let chainsA: [String: CoinSpec] = [
    ...
    // Harmony 是分片：三块分开的方片，沿对角线一级级架上去。原来是四枚圆扣成一圈，
    // 18px 上就是一个中间带洞的环，和 Ordinals 那枚打孔铭牌同一个骨架。真标那六个点
    // 排成六边形，六边形在 18px 上又会读回圆，所以这里改成对角排布、方角、数得出来。
    "ONE": CoinSpec(from: "#6FD8F2", to: "#1E8FB5", mark: fill([
      "M3.1 16.3h4.6v4.6H3.1z",
      "M9.7 9.7h4.6v4.6H9.7z",
      "M16.3 3.1h4.6v4.6h-4.6z"])),
    ...
    // Dash 是「数字现金」，跑得快：三道斜切的速度杠，也就是 Dash 那枚标里把 D 劈开的斜片。
    // 原来是「实心块挖一道横缝」的 D，18px 上和美光那三根存储条同一个骨架。
    "DASH": CoinSpec(from: "#6CBCF2", to: "#1C75BC", mark: fill([
      "M2.4 19.8h3.8l2.8-15.6H5.2z",
      "M8.7 19.8h3.8l2.8-15.6h-3.8z",
      "M15 19.8h3.8l2.8-15.6h-3.8z"])),
