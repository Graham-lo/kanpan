...
final class ChartViewport {
  ...
  ///
  /// 故意只暴露两个**动作**、不暴露任何状态：这层不该知道「有没有登录」「同步到哪一步了」
  /// 「存档长什么样」。什么时候调它们由这层决定（见 `settle()`）。
  ///
  /// 这儿曾经有第三个动作 `flushLayoutArchive()`（「把存档写盘队列排空，**等它写完
  /// 才返回**」），2026-09-22 去掉了。它在主线程上等的是**整份同步存档**的 JSON 编码
  /// 加原子写（实测 46 KB 起，画线多了几百 KB），每次抬手交一次；而它声称保住的那个值
  /// 根本不需要它保：`owner?.storeBarSpacing(want)` 走的是 `PrefsStore.persist()`，
  /// **同步**写 prefs.json 和脏标识，而且发生在这两个动作**之前**。杀了 app 再回来，
  /// 根宽是从 prefs.json 读的；存档那一版丢了也只是让「还没推上去」这件事重算一次
  /// （冷启动 `AppAccountBridge.prepare` 看见脏标识会重新记一条操作）。
  /// 真正「app 要走了」的那一刀在 `AppLifecycle` 的 `.sync` 档钩子上，排在所有产数据的
  /// 钩子后面，那一处仍然是阻塞排空。
  @MainActor protocol Sync: AnyObject {
    /// 把此刻这份设置记成一条待发操作，并**立刻发起**存档落盘（不等它写完）。
    /// **不去抖**——调它的时刻就是该记下的时刻。没真变过就什么都不记（由实现方自己比）。
    func recordLayout()
  ...
  ///    存档的落盘由它当场发起（后台串行队列），**这一层不在主线程上等它写完**：
  ///    等的是整份存档的编码加原子写，而第 2 步已经把用户看得见的那个值钉在盘上了；
  /// 4. 最后才谈推给服务端。推得上去最好，推不上去也不影响前三步——本地那份已经稳了。
  ///
