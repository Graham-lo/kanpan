...
enum FavoriteCategory {
  /// 「移到分类」里一直摆着的那几类（2026-09-24 删掉「新建分类」之后，第二类只能从这儿来）。
  ///
  /// 自建分类判了不做，而用户 09-20 定的规矩是「从某一类里点搜索加进来的品种留在那一类」，
  /// 两条加在一起，新用户手上就永远只有第一次加自选时开出来的那一类。所以按资产类型
  /// 自动起名的那几格在「移到分类」里常驻：还没开的，挑中了就当场开出来。
  /// 名字和 `name(symbol:info:)` 给的逐字一致，自动归类和手动移过去的会落进同一格。
  static let presets = ["加密", "美股", "贵金属"]
