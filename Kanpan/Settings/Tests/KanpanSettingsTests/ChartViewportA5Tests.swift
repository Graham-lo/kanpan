...
/// 两个动作（记操作 / 推服务端），一次 settle 恰好各一次，重复的抬手一次都不许多。
///
/// 2026-09-22：原来还有第三个动作「排空存档」（`flushLayoutArchive`），它在主线程上等
/// 整份同步存档编码 + 原子写，每次抬手交一次。那一版根宽在第 2 步就已经由
/// `PrefsStore.persist()` 同步写进盘里了，存档那一下不必在手指上等，所以协议里去掉了它。
@Suite("A.5 · 视野的落盘时机")
struct ChartViewportA5Tests {

  /// 把 `ChartViewport.Sync` 那两个动作数出来。这层不暴露任何状态，能观察的只有次数与顺序。
  @MainActor
  private final class CountingSync: ChartViewport.Sync {
    var records = 0, pushes = 0
    /// 调用顺序也记一份：`settle()` 的次序是「记操作 → 推」，反了就等于
  ...
  func case7_pendingAdoptSettle() {
    ...
    #expect((sync.records, sync.pushes) == (1, 1), "落盘 + 同步恰好一轮")
    #expect(sync.trace == ["record", "push"], "记操作必须排在推之前")
    ...
    #expect((sync.records, sync.pushes) == (1, 1), "pending 已清，抬手不该再提交一轮")
    viewport.interactionEnded()
    #expect((sync.records, sync.pushes) == (1, 1), "抬几次手都一样")
    ...
    #expect((sync.records, sync.pushes) == (1, 1), "到货不是用户动作，不提交")
  }
  ...
  func case7_settleIsIdempotent() {
    ...
    viewport.interactionEnded()
    #expect((sync.records, sync.pushes) == (1, 1), "没真变过就不提交")
  }
