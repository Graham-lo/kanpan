...
///
/// 风格表原来有十一款，这条用例也就横着跑十一遍；收成 AICoin 一套
...
struct ColorSampleTests {
  ...
  func sample() {
    ...
    for dark in [false, true] {
      ...
      guard let golden = goldenAll["aicoin"] else {
        Issue.record("原型色值里没有风格 aicoin")
        continue
      }
      var st = Evidence.state(dark: dark, size: dev.size)
      ...
      #expect(t.up.value.uppercased() == golden.upBody.uppercased(), "\(themeKey)/aicoin 涨色")
      #expect(t.down.value.uppercased() == golden.downBody.uppercased(), "\(themeKey)/aicoin 跌色")
      #expect(t.bg.value.uppercased() == golden.bg.uppercased(), "\(themeKey)/aicoin 底色")
      #expect(t.grid.value.uppercased() == golden.grid.uppercased(), "\(themeKey)/aicoin 网格色")
      #expect(t.dim.value.uppercased() == golden.axisText.uppercased(), "\(themeKey)/aicoin 轴文字色")
      ...
      #expect(
        ...
        "\(themeKey)/aicoin 时间轴线不在 timeY，位图可能翻了")
      ...
      for (name, want) in [("upBody", true), ("downBody", false)] {
        ...
        guard let c = xs.filter({ $0.up == want && $0.bodyHeight * s >= 6 })
          ...
          Issue.record("\(themeKey)/aicoin 找不到能取样的\(want ? "涨" : "跌")实体")
          continue
        }
        ...
        #expect(d == 0, "\(themeKey)/aicoin.\(name) 期望 \(col.value) 实得 \(hexOf(got))")
        rows.append(
          Sample(
      ...
      // ---------------------------------------------------------------- ③ 影线
      do {
        // AICoin 的影线不兑淡，和实体同色（见 `drawCandles`）。
        let col = t.up
        ...
        guard let c = xs.filter({ $0.up && ($0.bodyTop - $0.wickTop) * s >= 8 })
          ...
          Issue.record("\(themeKey)/aicoin 找不到够长的上影线")
          continue
        }
        // 影线一律填矩形，取左沿那一列。
        let xIdx = Int((c.wickLeft * s).rounded())
        ...
        #expect(
          best.d <= (full ? 0 : 1),
          "\(themeKey)/aicoin.wick 期望 \(hexOf(expected)) 实得 \(hexOf(best.got))")
        rows.append(
          Sample(
      ...
      do {
        ...
        if r.state.effectiveGrid == .none {
          ...
          #expect(!anyGrid, "\(themeKey)/aicoin 是 grid: none，不该画出网格")
          ...
          guard let y = ys.max(by: { abs($0 - lastY) < abs($1 - lastY) }) else {
            Issue.record("\(themeKey)/aicoin 一条网格线都没有")
            continue
          }
          let row = Int((hairline(y, scale: s) * s - 0.5).rounded())
          let got = px.rgb(xIdx, row)
          let d = chanDelta(got, t.grid.rgb8)
          #expect(d == 0, "\(themeKey)/aicoin.grid 期望 \(t.grid.value) 实得 \(hexOf(got))")
          rows.append(
            Sample(
      ...
      do {
        ...
        #expect(d == 0, "\(themeKey)/aicoin.bg 期望 \(t.bg.value) 实得 \(hexOf(got))")
        rows.append(
          Sample(
      ...
      do {
        guard let last = r.lastPriceY(size: dev.size) else {
          Issue.record("\(themeKey)/aicoin 最新价线不在主图里")
          continue
        }
        ...
        #expect(hit > 0, "\(themeKey)/aicoin.lastLine 这一行一个 \(col.value) 都没有")
      ...
      do {
        ...
        #expect(best.cov >= 0.5, "\(themeKey)/aicoin.axisText 最深的字像素才 \(best.cov) 覆盖")
        #expect(best.d <= 1, "\(themeKey)/aicoin.axisText 没落在底色→\(t.dim.value) 这条线上")
      ...
      #expect(rows.count == 7, "\(themeKey)/aicoin 只取到 \(rows.count) 个点")
      checked += rows.count
      for r0 in rows {
        csv.append(
          "\(themeKey),aicoin,\(r0.point),\(r0.expected),\(r0.actual),\(r0.delta),"
            + String(format: "%.4f", r0.coverage) + ",\(r0.exact)")
      }
      out.append([
        "theme": themeKey, "style": "aicoin", "name": "AICoin",
        "gridMode": r.state.effectiveGrid.rawValue, "shape": r.state.effectiveShape.rawValue,
