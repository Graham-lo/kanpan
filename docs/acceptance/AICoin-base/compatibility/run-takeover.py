...
def run(name):
    ...
    slug = name.replace(" ", "-").replace("(", "").replace(")", "") + "-one-screen-final"
    ...
    command = ["xcodebuild", "test-without-building", "-workspace", "Kanpan.xcworkspace",
               ...
               "-derivedDataPath", "DerivedData-codex-compat-takeover", "-parallel-testing-enabled", "NO",
    ...
    with lock:
        with (ROOT / "takeover-runs.jsonl").open("a") as out: out.write(json.dumps(summary, ensure_ascii=False) + "\n")
