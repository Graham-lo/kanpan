...
(ROOT/'current-r3-source-sha256.json').write_text(json.dumps(source,indent=2))

def run(name):
    if stop.is_set(): return
    device = next(d for d in devices if d['name'] == name)
    slug = name.replace(' ','-').replace('(','').replace(')','') + '-current-r3'
    ...
    with lock:
        with (ROOT/'current-r3-runs.jsonl').open('a') as out: out.write(json.dumps(result,ensure_ascii=False)+'\n')
