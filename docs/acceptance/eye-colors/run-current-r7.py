...
order = ['iPhone 15', 'iPhone 17e', 'iPhone 17 Pro', 'iPhone 17', 'iPhone 17 Pro Max', 'iPhone 16 Pro', 'iPhone 16 Plus', 'iPhone Air', 'iPad mini (A17 Pro)', 'iPad (A16)', 'iPad Air 11-inch (M4)', 'iPad Pro 11-inch (M5)']
...
(ROOT/'current-r7-source-sha256.json').write_text(json.dumps(source,indent=2))

def run(name):
    if stop.is_set(): return
    device = next(d for d in devices if d['name'] == name)
    slug = name.replace(' ','-').replace('(','').replace(')','') + '-current-r7'
    ...
    cmd = ['xcodebuild','test-without-building','-workspace','Kanpan.xcworkspace','-scheme','Kanpan','-destination','platform=iOS Simulator,id='+device['udid'],'-derivedDataPath','DerivedData-codex-current-r7','-parallel-testing-enabled','NO','-resultBundlePath',str(bundle)]
    ...
    with lock:
        with (ROOT/'current-r7-runs.jsonl').open('a') as out: out.write(json.dumps(result,ensure_ascii=False)+'\n')
