...
(ROOT/'current-r5-source-sha256.json').write_text(json.dumps(source,indent=2))

def run(name):
    if stop.is_set(): return
    device = next(d for d in devices if d['name'] == name)
    slug = name.replace(' ','-').replace('(','').replace(')','') + '-current-r5'
    ...
    cmd = ['xcodebuild','test-without-building','-workspace','Kanpan.xcworkspace','-scheme','Kanpan','-destination','platform=iOS Simulator,id='+device['udid'],'-derivedDataPath','DerivedData-codex-current-r5','-parallel-testing-enabled','NO','-resultBundlePath',str(bundle)]
    ...
    with lock:
        with (ROOT/'current-r5-runs.jsonl').open('a') as out: out.write(json.dumps(result,ensure_ascii=False)+'\n')
