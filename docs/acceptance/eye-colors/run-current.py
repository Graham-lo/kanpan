...
(ROOT/'current-source-sha256.json').write_text(json.dumps(source,indent=2))

def run(name):
    if stop.is_set(): return
    device = next(d for d in devices if d['name'] == name)
    slug = name.replace(' ','-').replace('(','').replace(')','') + '-current-r1'
    bundle = ROOT/(slug+'.xcresult')
    if bundle.exists():
        print('Preserve existing',name,flush=True); return
    tests = ['testComfortPalettesAndLayout','testFourSubpanelsFitWithoutPageScroll','testLatestEdgeAndReentryCompatibility']
    if name == 'iPhone 15': tests += ['testLatestQuoteDoesNotRegressAcrossIntervals','testQuickFavoritesAndMarketSectors']
    cmd = ['xcodebuild','test-without-building','-workspace','Kanpan.xcworkspace','-scheme','Kanpan','-destination','platform=iOS Simulator,id='+device['udid'],'-derivedDataPath','DerivedData-codex-current','-parallel-testing-enabled','NO','-resultBundlePath',str(bundle)]
    ...
    with lock:
        with (ROOT/'current-runs.jsonl').open('a') as out: out.write(json.dumps(result,ensure_ascii=False)+'\n')
