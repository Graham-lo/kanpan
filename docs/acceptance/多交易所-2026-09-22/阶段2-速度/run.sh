...
for r in 1 2 3; do
  measure after-gateway-r$r $A BTCUSDT 1h --route gateway
done
for r in 1 2 3; do
  measure cb-direct-r$r $A BTC-USD 1h --venue coinbase
done
else
for r in 1 2 3; do
  measure cb-gateway-r$r $A BTC-USD 1h --venue coinbase --route gateway
  measure cb-direct-post-r$r $A BTC-USD 1h --venue coinbase
done
fi
