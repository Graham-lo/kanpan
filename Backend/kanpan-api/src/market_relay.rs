...
//! * 同一来源地址同时在跑的中继（两条加起来）最多 `MAX_RELAYS_PER_CLIENT` 条，超了 429 +
...
/// 同一来源地址同时在跑的中继上限（两条中继合计）。一只币：币安两条（每条不超过 8 路流）+ OKX 一条
/// = 3 条，给到 4，留一条给换币时旧连接还没关干净的那一下。
const MAX_RELAYS_PER_CLIENT:usize=4;
