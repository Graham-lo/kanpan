...
pub async fn klines(provider:&dyn MarketDataProvider,r:&ChartRange,start:DateTime<Utc>,end:DateTime<Utc>)->Result<Value> {
 if r.venue=="binance" {return provider.klines(&r.market,&r.symbol,&r.interval,start,end).await.map_err(|_|ApiError(axum::http::StatusCode::SERVICE_UNAVAILABLE,"market_unavailable"))}
 ...
 for _ in 0..8 {
  let response=client.get("http://127.0.0.1:8792/market/v1/klines").query(&[("source","okx".to_owned()),("symbol",r.symbol.clone()),("interval",r.interval.clone()),("startTime",cursor.to_string()),("endTime",(end.timestamp_millis()-1).to_string()),("limit","500".into())]).send().await.map_err(|_|ApiError::bad("market_unavailable"))?.error_for_status().map_err(|_|ApiError::bad("market_unavailable"))?;
