...
- **自选页**（「琉璃」版，`Kanpan/Kanpan/Symbols/FavoritesView.swift`）：光斑底 + 玻璃纸，衬线标题旁一枚正放的数量印章，涨跌比例条；每行有品种徽章、价格与涨跌药丸（迷你走势图默认关闭，可在「…」里打开）；分类文件夹、排序（含涨跌幅口径）收进排序弹层；长按排序、滑动 / 批量删除、加号选品。冷启动有收藏就进自选页，否则进 BTC。
