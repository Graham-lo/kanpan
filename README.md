...
- **图表**：AICoin 手感复刻（[现行规格](docs/AICoin-K线复刻规格.md)）。主图均线（10/30/120/256）/ 指数均线 / 布林带，副图出厂成交量 + 持仓量 + 平滑异同，同时最多三个；十种候选还包括相对强弱、随机指标、随机强弱、真实波幅、多空比、主动买卖比、基差。直连支持四种外部统计，网关显示中文空态；「盘口」默认关，打开后按价位画买五卖五量柱；固定框双指缩放、历史焦点缩放、手动 Y、末根贴右缘、越界阻尼。41 把画线工具（九组，按 TradingView 手机版对齐），画线与图表共用坐标。
