...
- **皮肤**：「青苔·冷」（默认）、「陶土·暖」与「经典」三套，各有浅色 / 深色，跟随系统或手动指定。经典是青苔换了一张 AICoin 的白底（深色为 AICoin 夜间深蓝底 `#0D111C`），文字与强调色和青苔相同，但涨跌色、K 线、均线与副图指标线全部照搬 AICoin iPhone 端的实测值（蜡烛 `#36B257` / `#E64552`、副图槽位 `#2FD2B2 #FFB400 #E849B9 …`，深色取安卓包常量、未在真机量过），让图里图外同一张纸（`ThemeSkin` / `ThemeChoice`，色值种子在 `KanpanCore/Sources/KanpanCore/Style/Palette.swift`）。早期的靛色、纸色、暖暗配色已经不存在。
