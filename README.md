...
- **皮肤**：「青苔·冷」（默认）与「陶土·暖」两套，各有浅色 / 深色，跟随系统或手动指定（`ThemeSkin` / `ThemeChoice`，色值种子在 `KanpanCore/Sources/KanpanCore/Style/Palette.swift`）。早期的靛色、纸色、暖暗配色已经不存在。
...
- **自选页**（「琉璃」版，`Kanpan/Kanpan/Symbols/FavoritesView.swift`）：光斑底，列表行直接长在底上（2026-09-17 用户选了「融合」，不再垫玻璃纸），衬线标题旁一枚正放的数量印章，涨跌比例条；每行有品种徽章、价格与涨跌药丸（迷你走势图默认关闭，可在「…」里打开）；分类文件夹、排序（含涨跌幅口径）收进排序弹层；长按排序、滑动 / 批量删除、加号选品。冷启动有收藏就进自选页，否则进 BTC。
...
- **行情线路**：官方直连、两台 VPS 网关自动选路并持久保存；Binance 不可完整使用时整套切 OKX，后台低频确认恢复后切回。多品种启动快照与预热让冷启动和切换不等网络。
...
| `KanpanData/` | 行情 WS / REST、目录、历史 OI、选路与 Binance/OKX 切换 |
