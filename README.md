...
- **图表**：AICoin 手感复刻（[现行规格](docs/AICoin-K线复刻规格.md)）。主图 MA(10,30,120,256) / EMA / BOLL，副图出厂 VOL + OI + MACD（同时最多三个），可选 RSI、KDJ、StochRSI、ATR；固定框双指缩放、历史焦点缩放、手动 Y、末根贴右缘、越界阻尼。41 把画线工具（九组，按 TradingView 手机版对齐），画线与图表共用坐标。
