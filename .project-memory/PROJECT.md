...
- **周期条最多六档**（`Prefs.maxQuick = 6`，原来是 10）。那条「排不下就横向滚动、右边渐隐」的退路
  删掉了：能滚就意味着有钉住的档位藏在屏幕外。各档平分铺满整行；行尾是一个**三种状态一样宽**的固定槽位
  （「最新」/「返回刚才」/ 空），按最宽的那句话垫一颗影子药丸钉死，于是常用行拿到的宽度从头到尾不动
  （`IntervalSlotUITests` 量过：不钉死时一颗药丸露面就让 1d 往左跳 19.3pt）。
