...
- **服务端历史与回填**（2026-09-24 晚，Data `df659422` + `aa0ed623` → App `b34c6300`）：起订即取 kanpan-api 最近 24 小时并进模型（同簿同向同桶时间重叠以服务端为准，本机独有保留，步长对不上不并），每 60 秒取增量（from = 已取最新 − 5 分钟），图往左拖出已取区间按 24 小时一段补到 max(服务端开始跟的时刻, 现在 − 30 天)；服务端不通纯本地、不报错。内存模型留 30 天 / 2 万条（挤挂得最短的，保最近 2 小时与可视区），本机日志只存 24 小时 / 5000 条、更早的不落盘。冷启动 3.59 s 并进 2065 条（17 Pro Max，`OrderFlowHistoryUITests`）；服务端表每天约 14 万行 / 60 MB，30 天约 1.8 GB。方案与验收各有同名一节。
...
- 截图证据：`shots/`（审查，17 Pro Max）与 `整改/P0a|P0b|P1a|P1b|P2/`（16 Pro 与 17 Pro Max，只这两台）。
