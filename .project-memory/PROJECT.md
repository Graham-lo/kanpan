...
- 机型与系统兼容已做完一轮（2026-09-18，见 `docs/acceptance/兼容-iPhone15+-iPad-iOS18-2026-09-18.md`）：最低系统提到 iOS 18.0（各 SPM 包同步），iPad 按「不破」口径把满屏单列页封了 560pt 的内容列，全量 13 台矩阵已在 iOS 26.5 上跑完并把翻出来的十条红全部从根因修掉（其中 iPad 侧三条：双指缩放的 pt 死区、指标编辑纸被键盘顶飞吃掉第一下、横屏出口在 iPad 上问错了问题）。按用户口径不下载 iOS 18 运行时、真机不再跑。
