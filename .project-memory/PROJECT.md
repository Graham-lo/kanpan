...
- **第三批 I 线（进行中，`/tmp/kanpan-lanei`，分支 lane-i，iPhone 16 Pro）**：第 23 项 KanpanChart `ChartInput / Viewport / Overlay` 三层、画线收成 `DrawingFeature` 一份真相、Probe 复用真实路径、`GeometryCache` 只在几何输入变时失效；随后第 21 项 `ChartSession`。
- **后面**：第三批 22 / 26（J 线）、24 / 25 / 27，第 18 项目录搬家单独最后做。`docs/acceptance/**/*.xcresult` 有 153 个、9.2 GB（都未入库、超过一天），`rm -rf` 被分类器拦，待用户在终端跑 `find docs/acceptance -name '*.xcresult' -type d -prune -exec rm -rf {} +`，或以后把它加进 `scripts/machine-guard.sh clean`。
- **真机包**：签名账号仍断着，只在模拟器上验。
