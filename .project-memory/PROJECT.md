...
- **后面**：第四批 28–44 主力订单流整改（M 线客户端、N 线服务端与线路，N 线顺带查复盘找相似 15m 回空）；第 18 项目录搬家单独最后做。`docs/acceptance/**/*.xcresult` 有 153 个、9.2 GB（都未入库、超过一天），`rm -rf` 被分类器拦，待用户在终端跑 `find docs/acceptance -name '*.xcresult' -type d -prune -exec rm -rf {} +`，或以后把它加进 `scripts/machine-guard.sh clean`。
- **真机包**：签名账号仍断着，只在模拟器上验。
