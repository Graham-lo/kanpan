...
- **预算**（`scripts/machine-guard.sh`，launchd 看门狗每 60 秒巡一次）：同时最多 **2 个重编译**（xcodebuild / swift build|test / cargo）、**2 台开机模拟器**、磁盘空闲 **≥ 40 GB**、交换区 **≤ 8 GB**、空闲内存 **≥ 12%**。超预算的活自动排队，不是并行硬挤——排队比挤在一起换页快。**预算是动态的**：空闲内存 ≥ 35%、交换区 ≤ 4 GB、CPU 空闲 ≥ 25% 三项同时达标时，槽位自动放宽到 3 个编译 / 3 台模拟器；任一项掉下去新槽就不再发（在跑的不打断）。所以别自己算「机器还闲着」去绕开 `run`，让它排队就是在用闲置资源。
