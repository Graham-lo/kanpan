...
- **预算**（`scripts/machine-guard.sh`，launchd 看门狗每 60 秒巡一次）：构建槽按**点数**记——重活（xcodebuild 跑测试 / 带模拟器目标的构建）2 点、轻活（swift build|test、cargo、不带模拟器的 xcodebuild build）1 点，基础 **4 点**（= 2 个重活，或 1 重 + 2 轻，或 4 轻）；**开机模拟器最多 2 台**、磁盘空闲 **≥ 40 GB**、交换区 **≤ 8 GB**、空闲内存 **≥ 12%**。超预算的活自动排队，不是并行硬挤——排队比挤在一起换页快。**预算是动态的**：空闲内存 ≥ 35%、交换区 ≤ 4 GB、CPU 空闲 ≥ 25% 三项同时达标时自动放宽到 **6 点**（= 3 个重活）；任一项掉下去新槽就不再发（在跑的不打断）。再空一些——空闲内存 ≥ 50%——放宽到 **8 点**（= 4 个重活；用户 09-23 傍晚的话：「在保证不崩溃的情况下最大限度使用资源，不要浪费」）。09-23 实测 3 个重活 + 2 台模拟器就把空闲内存压到 31%、交换区往上长，所以内存在 35–50% 之间时 6 点封顶，瓶颈是内存不是 CPU。所以别自己算「机器还闲着」去绕开 `run`，让它排队就是在用闲置资源。
...
- **别在会话里裸跑编译，还有第二个理由——会话本身困在 macOS 的后台节流带里**（09-23 查「任务慢了很多」查出来的）：遥控宿主的 LaunchAgent `~/Library/LaunchAgents/com.mdd.kanpan.remote-control.plist` 写着 `ProcessType=Background`，宿主和它下面所有 Claude 会话、会话起的每个 xcodebuild / swift-frontend 都继承进节流带（`ps -o pri` 显示 4），Apple Silicon 只让这一带用 6 个能效核，4 个性能核一直空着——CPU 空闲常年 35–40% 就是这个。同一段 CPU 循环：会话里 5.64 s，launchd 起的 0.53 s；`nice`、`taskpolicy`、`setpriority`、`launchctl asuser` 都出不了这个带。所以 `scripts/machine-guard.sh run` 发现自己在节流带时（`pri < 10`）会把命令交给 `launchctl submit` 起（正常带 pri 20，仍 renice 10 让 Surge / ChatGPT / Claude 优先），尾随输出、取回退出码；`KANPAN_GUARD_DIRECT=1` 可强制直接跑。根治是把 plist 的 `ProcessType` 改成 `Standard` 再重启宿主（`launchctl kickstart -k gui/$(id -u)/com.mdd.kanpan.remote-control`，会掐掉所有在跑的会话），改完 `run` 自动不再走 launchd 那条路。以后装遥控宿主的 LaunchAgent 一律不写 `ProcessType=Background`。
