...
- 代码由 Claude 窗口实现（必要时派 Opus 5 子代理写、主窗口验收）。
- **例外（2026-09-22 起）：「发给朋友·画线分享」这一个功能由 Codex 实现**（`codex exec`，`gpt-6-astra`，`xhigh`），方案在 `docs/发给朋友-画线分享-实施方案-2026-09-22.md`，Claude 只设计与验收、不对就打回。Codex 在这个仓库里也遵守本文的全部约定。除此之外 Codex 不参与本项目；更早带 "Codex" 字样的文件与记忆都是历史。
