...
- **预算**（`scripts/machine-guard.sh`，launchd 看门狗每 60 秒巡一次）：构建槽按**点数**记——重活（xcodebuild 跑测试 / 带模拟器目标的构建）2 点、轻活（swift build|test、cargo、不带模拟器的 xcodebuild build）1 点，基础 **4 点**（= 2 个重活，或 1 重 + 2 轻，或 4 轻）；**开机模拟器最多 2 台**、磁盘空闲 **≥ 40 GB**、交换区 **≤ 8 GB**、空闲内存 **≥ 12%**。超预算的活自动排队，不是并行硬挤——排队比挤在一起换页快。**预算是动态的**：空闲内存 ≥ 35%、交换区 ≤ 4 GB、CPU 空闲 ≥ 25% 三项同时达标时自动放宽到 **6 点**（= 3 个重活）；任一项掉下去新槽就不再发（在跑的不打断）。09-23 实测 3 个重活 + 2 台模拟器就把空闲内存压到 31%、交换区往上长，所以 6 点是这台 16 GB 机器的上限，瓶颈是内存不是 CPU。所以别自己算「机器还闲着」去绕开 `run`，让它排队就是在用闲置资源。
